...
if 'tools' in st.session_state and st.session_state.tools:
    ...
    if 'confirming_deletion' in st.session_state:
        with st.expander("⚠️ Confirm Deletion", expanded=True):
            ...
            st.warning(f"Are you sure you want to delete `{st.session_state.tools[tool_index].name}`?")
...
custom_buttons = [
    {
        ...
        "commands": ["save-state", ["response","saved"]],    # List of commands (in this case, a custom command)
    }
]

# Add new tool or edit existing tool
st.write("### Add New Tool")
st.write('You can build a custimized tool right here. check out next session to see example.')
